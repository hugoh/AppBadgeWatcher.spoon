-- Test suite for AppBadgeWatcher.spoon using Busted
-- Run with: busted test_spec.lua
    ...
    mock_hs = {
        ...
        canvas = {
            new = function(frame)
                ...
                canvas.imageFromCanvas = function(self)
                    return { elements = elements, frame = frame }
                end
        ...
        menubar = {
            new = function()
                return {
                    setTitle = function(self, title) self._title = title return self end,
                    setIcon = function(self, icon, flag) self._icon = icon return self end,
                    setClickCallback = function(self, cb) self._clickCb = cb return self end,
                    delete = function(self) self._deleted = true return self end,
                }
            end
        },
        timer = {
...
describe("AppBadgeWatcher", function()
    ...
    describe("module structure", function()
        it("returns a table", function()
            assert.is.table(AppBadgeWatcher)
        end)
        
        it("has correct name", function()
            assert.are.equal("AppBadgeWatcher", AppBadgeWatcher.name)
        end)
        
        it("has correct version", function()
            assert.are.equal("1.0.1", AppBadgeWatcher.version)
        end)
        
        it("has correct author", function()
            assert.are.equal("Hugo Haas", AppBadgeWatcher.author)
        end)
        
        it("has correct license", function()
            assert.are.equal("MIT", AppBadgeWatcher.license)
        end)
        
        it("has homepage URL", function()
            assert.are.equal("https://github.com/hugoh/AppBadgeWatcher.spoon", AppBadgeWatcher.homepage)
        end)
    end)
    ...
    describe("default configuration", function()
        ...
        it("has default refreshInterval of 15", function()
            assert.are.equal(15, AppBadgeWatcher.refreshInterval)
        end)
        
        it("has default nothingIndicator", function()
            assert.are.equal("・", AppBadgeWatcher.nothingIndicator)
        end)
        
        it("has default grayscaleIcon as false", function()
            assert.is_false(AppBadgeWatcher.grayscaleIcon)
        end)
        
        it("has default fontSize of 6", function()
            assert.are.equal(6, AppBadgeWatcher.fontSize)
        end)
        
        it("has default textOffset", function()
            assert.are.equal(2, AppBadgeWatcher.textOffset.x)
            assert.are.equal(0, AppBadgeWatcher.textOffset.y)
        end)
    end)
    ...
    describe("updateMenu", function()
        ...
        it("clears snoozedBadges when no badges", function()
            ...
            AppBadgeWatcher.getDockBadges = function(self)
                return {}
            end
            AppBadgeWatcher:updateMenu(true)
    ...
    describe("configuration changes", function()
        it("allows updating appsToWatch", function()
            AppBadgeWatcher.appsToWatch = { "Mail", "Slack", "Teams" }
            assert.are.equal(3, #AppBadgeWatcher.appsToWatch)
        end)
        
        it("allows updating refreshInterval", function()
            AppBadgeWatcher.refreshInterval = 30
            assert.are.equal(30, AppBadgeWatcher.refreshInterval)
        end)
        
        it("allows updating nothingIndicator", function()
            AppBadgeWatcher.nothingIndicator = "○"
            assert.are.equal("○", AppBadgeWatcher.nothingIndicator)
        end)
        
        it("allows updating grayscaleIcon", function()
            AppBadgeWatcher.grayscaleIcon = true
            assert.is_true(AppBadgeWatcher.grayscaleIcon)
        end)
        
        it("allows updating fontSize", function()
            AppBadgeWatcher.fontSize = 8
            assert.are.equal(8, AppBadgeWatcher.fontSize)
        end)
        
        it("allows updating textOffset", function()
            AppBadgeWatcher.textOffset = { x = 3, y = 1 }
            assert.are.equal(3, AppBadgeWatcher.textOffset.x)
            assert.are.equal(1, AppBadgeWatcher.textOffset.y)
        end)
    end)
    ...
    describe("internal state", function()
        it("initializes with nil timer", function()
            assert.is_nil(AppBadgeWatcher.timer)
        end)
        
        it("initializes with nil menu", function()
        ...
        it("has logger instance", function()
            assert.is_table(AppBadgeWatcher.log)
        end)
    end)
end)
